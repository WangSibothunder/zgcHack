...
2. 医生时间轴展示三位合成转院患者，心内科病例有 6 个既有节点。
3. 每个节点都能回到原始合成材料、OCR 和证据位置。
4. 采集页先上传模糊合成材料，系统提示图片质量不足，需要重拍。
5. 再上传清晰合成化验单，系统完成质量检测、OCR fallback、字段抽取和时间轴生成。
6. 新节点进入医生时间轴，点击字段即可在原图看到高亮证据框。
7. 医生确认正确或标记需复核，系统保存核验记录。
8. 接诊前摘要展示材料覆盖、关键节点、核验计数和材料完整性提示。
9. 结尾说明：本 Demo 只验证合成材料采集、整理和证据追溯流程，不生成诊断或治疗建议。
